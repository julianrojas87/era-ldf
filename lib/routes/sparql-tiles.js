...
export async function sparqlTiles(fastify, options) {
    ...
    fastify.get(`/${urlBase}/sparql-tiles/implementation/:z/:x/:y`, async (request, reply) => {
        const headers = { accept: request.headers['accept'] || implementationTiles.accept };
        ...
        if (config.concurrentQueries) {
            await Promise.all(implementationTiles.queries.map(async q => {
                const sparql = q(lat1, lon1, lat2, lon2);
    ...
    fastify.get(`/${urlBase}/sparql-tiles/abstraction/:z/:x/:y`, async (request, reply) => {
        const headers = { accept: request.headers['accept'] || abstractionTiles.accept };
        ...
        if (config.concurrentQueries) {
            await Promise.all(abstractionTiles.queries.map(async q => {
                const sparql = q(lat1, lon1, lat2, lon2);
                logger.debug(`Executing the following SPARQL query: \n${sparql}`);
            ...
            for (const q of abstractionTiles.queries) {
                const sparql = q(lat1, lon1, lat2, lon2);
                logger.debug(`Executing the following SPARQL query: \n${sparql}`);
